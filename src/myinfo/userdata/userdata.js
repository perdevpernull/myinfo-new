import './userdata.2017-do-not-delete.jsondb';

function userdata (jsondb) {
  const _private_ = {};

  class UserData {
    constructor (jsondb) {
      if (jsondb && jsondb.nextDatasetID) {
    ...
    getDatasets () {
      return _private_.data.datasets;
    }

    getDataset (datasetKey) {
      return _private_.data.datasets[datasetKey];
    }

    addDataset (datasetTitle, datasetDescription, datasetLink) {
      var datasetID = _private_.data.nextDatasetID;
      _private_.data.nextDatasetID = datasetID + 1;
      var datasetKey = 'ID' + datasetID;
      if (!datasetLink) {
        datasetLink = '/api/v1/dataset';
      }

      _private_.data.datasets[datasetKey] = {
        ID: datasetID,
    ...
    getDatasetInstance (datasetKey) {
      if (!_private_.dataTmp.datasets[datasetKey]) {
        _private_.dataTmp.datasets[datasetKey] = { instance: null, views: {} };
      }
      return _private_.dataTmp.datasets[datasetKey].instance;
    }

    setDatasetInstance (datasetKey, datasetInstance) {
      if (!this.getDatasetInstance(datasetKey)) {
        _private_.dataTmp.datasets[datasetKey].instance = datasetInstance;
      } else {
    ...
    getView (datasetKey, viewKey) {
      if (!viewKey) {
        viewKey = `ID${_private_.data.datasets[datasetKey].selectedViewIndex}`;
      }
      return _private_.data.datasets[datasetKey].views[viewKey];
    }

    setLayoutPluginInstance (datasetKey, viewKey, layoutPluginInstance) {
      if (!this.getLayoutPluginInstance(datasetKey, viewKey)) {
        _private_.dataTmp.datasets[datasetKey].views[viewKey].instance = layoutPluginInstance;
      } else {
    ...
    getLayoutPluginInstance (datasetKey, viewKey) {
      ...
      if (view) {
        ...
        if (!_private_.dataTmp.datasets[datasetKey].views[`ID${view.ID}`]) {
          _private_.dataTmp.datasets[datasetKey].views[`ID${view.ID}`] = { instance: null };
        }
        instance = _private_.dataTmp.datasets[datasetKey].views[`ID${view.ID}`].instance;
      }
      return instance;
    }
  }

  return new UserData(jsondb);
}
